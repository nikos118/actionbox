...
describe("matchToolCall (multi-skill global policy)", () => {
  it("returns no violations for an allowed tool", () => {
    ...
    const violations = matchToolCall("google_calendar_read", {}, policy);
    expect(violations).toHaveLength(0);
  });

  it("returns a critical violation for a globally denied tool", () => {
    // shell_exec is denied by both contracts and allowed by none
    const policy = buildGlobalPolicy([calendarBox, githubBox]);
    const violations = matchToolCall("shell_exec", {}, policy);
  ...
  it("does NOT deny a tool that one contract denies but another allows", () => {
    ...
    const violations = matchToolCall("github_close_issue", {}, policy);
  ...
  it("returns a high violation for an unlisted tool", () => {
    const policy = buildGlobalPolicy([calendarBox]);
    const violations = matchToolCall("unknown_tool", {}, policy);
  ...
  it("detects globally denied filesystem paths", () => {
    const policy = buildGlobalPolicy([calendarBox, githubBox]);
    // ~/.ssh/** is denied by both contracts
    const violations = matchToolCall("google_calendar_read", {
      path: "~/.ssh/id_rsa",
    }, policy);
  ...
  it("allows reads to paths permitted by claiming contract", () => {
    const policy = buildGlobalPolicy([calendarBox]);
    const violations = matchToolCall("google_calendar_read", {
  ...
  it("flags reads to paths not permitted by any contract", () => {
    const policy = buildGlobalPolicy([calendarBox]);
    const violations = matchToolCall("google_calendar_read", {
      path: "/etc/passwd",
    }, policy);
  ...
  it("allows a path if ANY claiming contract permits it (multi-skill)", () => {
    // slack_send_message is claimed by both calendar and github contracts
    // ./config/** is readable by both
    const policy = buildGlobalPolicy([calendarBox, githubBox]);
    const violations = matchToolCall("slack_send_message", {
      path: "./config/app.yaml",
    }, policy);
    expect(violations).toHaveLength(0);
  });

  it("detects write to non-writable path for a claimed tool", () => {
    ...
    const violations = matchToolCall("write_file", {
      path: "./config/calendar.yaml",
    }, policy);
  ...
  it("flags unlisted tool that tries to write", () => {
    const policy = buildGlobalPolicy([calendarBox]);
    const violations = matchToolCall("write_file", {
      path: "./config/calendar.yaml",
    }, policy);
  ...
  it("detects network violations via URL param extraction", () => {
    const policy = buildGlobalPolicy([calendarBox]);
    const violations = matchToolCall("google_calendar_read", {
  ...
  it("detects globally denied network hosts", () => {
    const policy = buildGlobalPolicy([calendarBox, githubBox]);
    const violations = matchToolCall("google_calendar_read", {
  ...
  it("returns no violations when no contracts loaded", () => {
    const policy = buildGlobalPolicy([]);
    const violations = matchToolCall("anything", { path: "~/.ssh/id_rsa" }, policy);
    expect(violations).toHaveLength(0);
  });
});
